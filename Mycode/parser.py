...
class Parser:
    """
        update - полностью обновить и сохранить результат
        select - выбрать из csv таблицы по датам и компании/валюте
    """

    def __init__(self):
        self.currency_links = {'USD': 'R01235', 'CNY': 'R01375', 'EUR': 'R01239', 'GBP': 'R01035'}
        self.companies = ['YNDX', 'SBER', 'TCSG', 'GAZP', 'RNFT', 'ROSN', 'SNGSP', 'NVTK', 'AFKS', 'FIVE', 'MGNT']
        self.today_date = str(datetime.now().date().strftime('%d-%m-%Y'))
    ...
    def update_central_bank_rate(self):
        ...
        html = requests.get(f'https://cbr.ru/hd_base/KeyRate/?UniDbQuery.Posted=True&UniDbQuery.From={start_date}'
                            f'&UniDbQuery.To={self.today_date.replace("-", ".")}').text
    ...
    def select_central_bank_rate(self, start_date, end_date):
    ...
    def select_inflation_rate(self, start_month, end_month):
    ...
    def update_currency_exchange_rate(self):
        for curr in self.currency_links.keys():
            ...
            html = requests.get(f'https://www.cbr.ru/currency_base/dynamics/?UniDbQuery.Posted=True&UniDbQuery.so=1&'
                                ...
                                f'VAL_NM_RQ={self.currency_links[curr]}&UniDbQuery.From=01.01.2000&'
                                f'UniDbQuery.To={self.today_date}').text
            soup = BeautifulSoup(html, 'html.parser')
            tables = str(soup.find_all('table')).split()
            dates = []
            rates = []
    ...
    def select_currency_exchange_rate(self, currency, start_date, end_date):
        df = pd.read_csv(f'data/currency/{currency}_exchange_rate.csv')
        return df.iloc[df.index[df['date'] == end_date].tolist()[0]:df.index[df['date'] == start_date].tolist()[0] + 1]

    async def update_shares_rate(self):
        for cmpny in self.companies:
    ...
    def select_shares_rate(self, company, start_date, end_date):
