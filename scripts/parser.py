...
from .CONST import CURRENCY_LINKS, SHARES_NAMES
    ...
    for i in range(len(tables)):
        if tables[i][:4] == '<td>' and tables[i][-5:] == '</td>':
            if tables[i].find('.') > -1:
                ...
                for i in range(second_day, first_day, -1):
                    if i < 10:
...
async def update_shares_rate():
    for share in SHARES_NAMES.keys():
        print('update ' + share + ' shares rate')
        async with aiohttp.ClientSession() as session:
            data = list(await aiomoex.get_board_history(session, share))
            fixed_data = []
            last_date = ''
            for line in data:
                if last_date == '':
            ...
            if os.path.exists(f'./data/shares/{share}_shares_rate.csv'):
                os.remove(f'./data/shares/{share}_shares_rate.csv')
            df.to_csv(f'data/shares/{share}_shares_rate.csv')
