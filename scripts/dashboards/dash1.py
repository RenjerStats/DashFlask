...
from ..parser import select_currency_exchange_rate
from ..CONST import CURRENCY_NAMES


def create_dash1(requests_pathname_prefix):
    dash = BaseDashboard(requests_pathname_prefix, "Курсы валют")
    dropdown_curses = dcc.Dropdown(
        options=CURRENCY_NAMES,
        value=[list(CURRENCY_NAMES.keys())[0]],
        id='dropdown_curses',
        multi=True,
        clearable=False,
        optionHeight=50,
...
def UI(app):
    ...
    def update_date(count_click, str_start, str_end, curses):
        ...
        if not curses:
            return px.line()
        data = [select_currency_exchange_rate(curse, str_start, str_end).assign(name=curse) for curse in curses]
        df = pd.concat(data)
